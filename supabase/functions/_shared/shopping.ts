...
const NOT_GROCERY_RE =
  /\b(present|gift|birthday|party|appointment|dentist|doctor|teacher|email|call|book|haircut|babysitter|shoes|wedding|clothes|uniform|accommodation|hotel|lodging|stay|ticket|flight|passport|form|permission|rsvp|session|bootcamp|trip|holiday|vacation|childcare|childminder)\b/i;

const TASK_VERB_RE =
  /\b(book|call|sign|apply|return|confirm|rsvp|renew|appointment|checkup|passport|form|permission|arrange|organise|organize|sort)\b/i;
...
export function looksLikeGroceryProduct(title: string, category?: string | null): boolean {
  ...
  if (NOT_GROCERY_RE.test(t) || TASK_VERB_RE.test(t)) return false;
  if (isGroceryCapture(t)) return true;
  const words = t.split(' ').filter(Boolean);
  if (words.length === 1 && /^[a-zA-Z][a-zA-Z'-]*$/.test(t)) return true;
  if (words.length === 2 && (category === 'errand' || category === 'delivery') && !/\bfor\b/i.test(t)) {
    return true;
  }
  return false;
}
...
export function isGroceryCapture(text: string): boolean {
  ...
  if (/\bneed to\b/i.test(t) || NOT_GROCERY_RE.test(t) || TASK_VERB_RE.test(t)) return false;
